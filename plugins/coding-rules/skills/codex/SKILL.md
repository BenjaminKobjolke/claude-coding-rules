...
Controls whether the AI workflow rules in this project delegate to Codex (via
`tools/coding_rules_delegate.sh codex ...`) or have the agent perform the same
checks itself (`/plan:dry`, `/convention:check`,
`/dry:check`).
...
4. **Install the delegate wrapper.** Copy both files from
   ...
   Both backends live in the one script, so it is the same file DeepSeek uses.
...
2. Live check. Write a prompt file (Write tool) containing
   `Reply with exactly: CODEX_OK`, then from the project directory run:

   ```
   tools/coding_rules_delegate.sh codex "<prompt-file>" "<log-file>"
   ```
...
3. Report: one pass/fail line per check, with a one-line fix hint per
   failure — `run /coding-rules:codex on` for marker, wrapper or permission
   failures, install / `codex login` for CLI failures.
